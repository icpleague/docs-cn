...
....
+
如果您没有更改用于运行 `+dfx deploy+` 命令的身份，则应该通过运行 `+dfx identity get-principal+` 命令获得相同的主体。
...
(0)
....
. 通过运行类似于以下的命令，使用容器主体将一些周期从您的默认钱包容器发送到 `+cycles_hello+` 容器:
+
[source,bash]
----
