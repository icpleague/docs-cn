...
* 您是新的 ICP 代币持有者。
* 您想了解您可以使用 ICP 代币做什么。
* 您想知道如何使用 {sdk-short-name} 命令行界面 DFX 转换、转移或锁定您的 ICP 令牌。

如果您还不是代币持有者，您需要从交易所购买 ICP 代币或获得代币授权，然后才能进行托管。有关如何获取 ICP 代币和托管选项的概述，请参见link：../developers-guide/concepts/tokens-cycles{outfilesuffix}#get-cycles[如何获取 ICP 代币] 和链接：托管选项- intro{outfilesuffix}【选择数字资产自托管】。

如果您正在使用其他应用程序（例如链接：https://nns.ic0.app[Network Nervous System (NNS) application] 或硬件钱包提供的用户界面）与您的 ICP 代币进行交互，您应该参考到该应用程序的文档。

这个 _自托管快速入门_ 只专注于使用 {sdk-short-name} 命令行界面 DFX 与 ICP 令牌进行交互。
...
* 您有一个安全的环境来执行涉及 ICP 令牌的操作。
+
作为安全最佳实践，任何涉及 ICP 令牌传输的操作都需要一台具有最少硬件和软件的气隙计算机以及一台连接到网络的计算机。
...
. 通过运行类似于以下的命令从您的帐本帐户转移 ICP 令牌，创建一个具有燃料费的新容器：
+
[source,bash]
----
...
....
+
此示例说明如何使用带有 `+--icp+` 命令行选项的整数将 ICP 令牌转移到指定帐户。
+
--

* 您还可以使用 `+--e8s+` 选项单独或与 `+--icp+` 选项一起指定 ICP 令牌的小数单位 - 称为 *e8s*。
...
....
. 通过运行类似于以下的命令，将 ICP 令牌从分类帐帐户转移到上一步中返回的目标帐户标识符：
+
[source,bash]
----
